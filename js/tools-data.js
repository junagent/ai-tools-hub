...
function setToolOGMeta(tool) {
    ...
    // 注入相关工具推荐 JSON-LD (isRelatedTo)
    if (tool.alternatives && tool.alternatives.length > 0 && typeof TOOLS_DATA !== 'undefined') {
        var relatedItems = tool.alternatives.map(function(altId) {
            var alt = TOOLS_DATA.find(function(t) { return t.id === altId; });
            if (!alt) return null;
            return {
                '@type': 'SoftwareApplication',
                'name': alt.name,
                'description': alt.description,
                'url': 'https://junagent.github.io/ai-tools-hub/tools/' + alt.id + '.html'
            };
        }).filter(Boolean);
        ...
        if (relatedItems.length > 0) {
            var relatedLd = {
                ...
                '@type': 'SoftwareApplication',
                'name': tool.name,
                'isRelatedTo': relatedItems
            };
            var relScript = document.createElement('script');
            relScript.type = 'application/ld+json';
            relScript.textContent = JSON.stringify(relatedLd);
